...
function drawTrajectory(previous, current) {
  ...
  drawLegend("履歴ライン", "rgba(95, 147, 196, 0.45)", legendBaseX, [8, 6]);
...
function createSingleForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (previousResult) {
      refs.maxHeightResultPrevious.textContent = `履歴: ${previousResult.maxHeightMeters.toFixed(1)} m`;
      refs.carryResultPrevious.textContent = `履歴: ${(previousResult.carryMeters / METERS_PER_YARD).toFixed(1)} yd`;
      refs.totalResultPrevious.textContent = `履歴: ${(previousResult.totalMeters / METERS_PER_YARD).toFixed(1)} yd`;
    } else {
      refs.maxHeightResultPrevious.textContent = "履歴: -";
      refs.carryResultPrevious.textContent = "履歴: -";
      refs.totalResultPrevious.textContent = "履歴: -";
    }
