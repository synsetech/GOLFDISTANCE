...
function computeRunWithBounceFromLanding(landingVx, landingVy, spinLandRpm) {
  ...
  const spinLoss = clamp(1 - 0.22 * spinNorm * spinSign, 0.72, 1.0);
  ...
  const rollMeters = computeRunMetersFromLanding(vx, 0);
...
function createSide(prefix, sideResultSetter) {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (error) {
      showError(error);
      refs.maxHeightResult.textContent = "-";
      refs.carryResult.textContent = "-";
      refs.totalResult.textContent = "-";
    ...
    refs.maxHeightResult.textContent = `${result.maxHeightMeters.toFixed(1)} m`;
    refs.carryResult.textContent = `${(result.carryMeters / METERS_PER_YARD).toFixed(1)} yd`;
    refs.totalResult.textContent = `${(result.totalMeters / METERS_PER_YARD).toFixed(1)} yd`;
