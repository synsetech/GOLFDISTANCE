...
const SIM_CONFIG = getSimulationConfig();
...
function validateInputs(headSpeedRaw, smashFactorRaw, launchAngleRaw, spinRateRaw, windSpeedRaw) {
  ...
  if (headSpeed < SIM_CONFIG.headSpeed.min || headSpeed > SIM_CONFIG.headSpeed.max) return `ヘッドスピードは ${SIM_CONFIG.headSpeed.min.toFixed(1)}〜${SIM_CONFIG.headSpeed.max.toFixed(1)} の範囲で入力してください。`;
  if (smashFactor < SIM_CONFIG.smashFactor.min || smashFactor > SIM_CONFIG.smashFactor.max) return `ミート率は ${SIM_CONFIG.smashFactor.min.toFixed(2)}〜${SIM_CONFIG.smashFactor.max.toFixed(2)} の範囲で入力してください。`;
  if (launchAngle < SIM_CONFIG.launchAngle.min || launchAngle > SIM_CONFIG.launchAngle.max) return `打ち出し角は ${SIM_CONFIG.launchAngle.min.toFixed(1)}〜${SIM_CONFIG.launchAngle.max.toFixed(1)} 度の範囲で入力してください。`;
  if (spinRate < SIM_CONFIG.spinRate.min || spinRate > SIM_CONFIG.spinRate.max) return `スピンレートは ${SIM_CONFIG.spinRate.min}〜${SIM_CONFIG.spinRate.max} rpm の範囲で入力してください。`;
  if (windSpeed < SIM_CONFIG.windSpeed.min || windSpeed > SIM_CONFIG.windSpeed.max) return `風向風速は ${SIM_CONFIG.windSpeed.min.toFixed(1)}〜+${SIM_CONFIG.windSpeed.max.toFixed(1)} m/s の範囲で入力してください。`;
